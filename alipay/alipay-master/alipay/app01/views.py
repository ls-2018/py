...
def aliPay():
    obj = AliPay(
        ...
        return_url=settings.RETURN_URL,  # 如果支付成功，重定向回到你的网站的地址。（公网地址）
...
def index(request):
    ...
    # 价格、购买得商品加密,这就用到了util中得pay.py(sdk)加密并生成url
    # 拼接成URL
...
def pay_result(request):
    ...
    status = alipay.verify(params, sign)  # 校验是否是 阿里得 签名
