...
def create_account():
    ...
    if request.vars.username and request.vars.web2py_admin_password:
        ...
        client = ServerProxy(
            'https://%(username)s:%(web2py_admin_password)s@%(username)s.pythonanywhere.com/admin/webservices/call/jsonrpc' % request.vars)
        try:
    ...
    try:
        ...
        if error.code == 400:
            ...
            return response.json({'status': 'error', 'errors': {'username': [
                'An App other than web2py is installed in the domain %(username)s.pythonanywhere.com' % request.vars]}})
        else:
            raise
...
def list_apps():
    ...
    if not request.vars.username or not request.vars.password:
        raise HTTP(400)
    client = ServerProxy(
        'https://%(username)s:%(password)s@%(username)s.pythonanywhere.com/admin/webservices/call/jsonrpc' % request.vars)
    regex = re.compile('^\w+$')
