    ...
    def test_sanitize(self):
        ...
        for x in permitted_tags:
            ...
            if x == "img/":  # alt or src attribute is required. src has to have a valid href
                ...
                self.assertEqual(XML(s_tag, sanitize=True, permitted_tags=['img/'],
                ...
                self.assertEqual(
                    XML(s_tag, sanitize=True, permitted_tags=['a'], allowed_attributes={'a': ['href', 'title']}).xml(),
                ...
                self.assertEqual(XML(s_tag, sanitize=True, permitted_tags=permitted_tags,
        ...
        for x in out_of_list:
            T = TAG[x]
            self.assertEqual(XML(T().xml(), sanitize=True, permitted_tags=permitted_tags,
                                 allowed_attributes=allowed_attributes).xml(), "&lt;%s&gt;&lt;/%s&gt;" %
                             (x, x))
        # test unusual tags
        ...
        self.assertEqual(
            XML(s_tag, sanitize=True, permitted_tags=['img/'], allowed_attributes={'img': ['src', 'alt']}).xml(),
        ...
        self.assertEqual(XML(TAG['a']("oh no!", _href="invalid_link").xml(), sanitize=True, permitted_tags=['a']).xml(),
                         'oh no!')
        self.assertEqual(XML(TAG['div']("", _onclick="evil()").xml(), sanitize=True, permitted_tags=['div']).xml(),
                         '<div></div>')
        ...
        self.assertEqual(XML(TAG['a'](TAG['img/'](_src="/index.html"), _class="teste").xml(), sanitize=True,
