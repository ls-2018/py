...
class SQLFORM(FORM):
    ...
    def factory(*fields, **attributes):
        ...
        dummy_dal = DAL(None)
        dummy_dal.validators_method = lambda f: default_validators(dummy_dal,
    ...
    def grid(query,
        ...
        exportManager = dict(
            csv_with_hidden_cols=(ExporterCSV_hidden, 'CSV (hidden cols)',
                                  T('Comma-separated export including columns not shown; fields from other tables are exported as raw values for faster export')),
            csv=(ExporterCSV, 'CSV',
                 T('Comma-separated export of visible columns. Fields from other tables are exported as they appear on-screen but this may be slow for many rows')),
            ...
            tsv_with_hidden_cols=(ExporterTSV_hidden, 'TSV (Spreadsheets, hidden cols)',
                                  T('Spreadsheet-optimised export of tab-separated content including hidden columns. May be slow')),
    ...
    def __init__(self,
        ...
        if columns:
            ...
            for tablename in set(tablenames):
                ...
                fieldmap.update(
                    (("%s.%s" % (tablename, f.name), f) for f in table._virtual_fields + table._virtual_methods))
        else:
