    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            try:
                ...
                print("{} span".format(service_name), span_record)
                q.task_done()
                print('event loop')
...
def before_request(request):
    ...
    span = opentracing.tracer.start_span(operation_name=handler[0].__name__, child_of=span_context)
...
def create_span(span_id, parent_span_id, trace_id, span_name, start_time, duration, annotations, binary_annotations):
    return {
        'traceId': trace_id,
        'name': span_name,
        'id': span_id,
