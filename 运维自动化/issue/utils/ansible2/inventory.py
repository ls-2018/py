...
class Host(BaseHost):
    def __init__(self, host_data):
        ...
        self.hostname = host_data.get('hostname') or host_data.get('ip')
        self.port = host_data.get('port') or 22
        super(Host, self).__init__(self.hostname, self.port)
...
class Inventory(BaseInventoryManager):
    """
    提供生成Ansible inventory对象的方法
    """
    loader_class = DataLoader
    variable_manager_class = VariableManager
    host_manager_class = Host
    ...
    def __init__(self, host_list=None):
        ...
        super(Inventory, self).__init__(self.loader)
