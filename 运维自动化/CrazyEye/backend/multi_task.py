...
class MultiTask(object):
    ...
    def __run_cmd(self, msg):
        ...
        if self.__parse_argv(msg):
            ...
            if task_host_list:
                while True:
                    user_choice = input(
                        "Are you sure you want to run [\033[33;1m%s\033[0m] command on above hosts？（Y/N）" % msg[
                            msg.index('-cmd') + 1]).strip()
    ...
    def __fetch_hosts(self, msg):
        ...
        remote_users = self.main_ins.ms.select("select id,username,password from host_users where username=%s",
                                               (remote_username,))
        if not remote_users:
        ...
        if '-g' in msg:
            ...
            and user.id = %s
            and host_users.username = %s '''
            all_avaliable_hosts = self.main_ins.ms.select(all_avaliable_hosts_query,
                                                          (self.main_ins.user_id, remote_username))
                        ...
                        if msg[0] == 'run_cmd':
                            ...
                            if not '-put' in msg or not '-get' in msg:
                                ...
                                if '-put' in msg:
                                    ...
                                    if '-get' in msg:
                                        if not len(msg[msg.index('-get'):]) > 1 or \
                                                msg[msg.index('-get') + 2].startswith('-'):
