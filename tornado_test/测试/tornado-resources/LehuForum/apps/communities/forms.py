...
class PostForm(Form):
    title = StringField("标题", validators=[DataRequired("请输入帖子标题")])
    content = TextAreaField("内容", validators=[DataRequired(message="请输入帖子内容")])
...
class ApplysForm(Form):
    status = StringField("处理结果", validators=[AnyOf(values=['agree', 'refuse'])])
